...
var trans_Q = [];
var detector_Q = [];
var nDiscaredTransactions = 0;
var nDiscaredDetectorResults = 0;
...
setInterval(function()
{
	...
	if (trans_Q.length != 0)
	{
		...
		if(!process_transactions_url) {
			nDiscaredTransactions += trans.transactions.length;
		} else {
	...
	if (detector_Q.length != 0)
	{
		...
		if(!process_detectors_url) {
			nDiscaredDetectorResults += detect.detector_results.length;
		} else {
			var xhttp = new XMLHttpRequest();
			xhttp.onreadystatechange = function() {
				if (this.readyState == 4 && this.status == 200) {
